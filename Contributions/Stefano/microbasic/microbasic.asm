; Tiny (3K) integer Basic by Dave Dunfield for the
; Univeristy of New Brunswicks MicroComputer group
;
; Ported to Z80 (and Intel) by Stefano Bodrato
;
;
;TITLE	MICRO BASIC
;**************************************************************
...
;	and a
;	JP	Z,INLF		;IF ESC, ABORT BACK TO COMMAND HANDLER
...
;* COPY NEW LINE INTO OLD LINE BUFFER (MAKEING IT THE 'NEW' OLD LINE)
	...
	LD	HL,BUFF	;POINT TO OLD LINE BUFFER
.MOVL1	LD	A,(DE)		;GET CHARACTER FROM NEW LINE
	LD 	(HL),A		;SAVE IN OLD LINE BUFFER
	INC	HL		;POINT TO NEXT
	INC	DE		;POINT TO NEXT
	CP	$0D		;TEST FOR END OF LINE
	JP	NZ,MOVL1		;IF NOT, KEEP MOVEING
...
;*
.LOAD	CALL	EXPR		;GET EXPRESSION
	JP	NC,SYNT	;IF NUMERIC, Syntaz error !
...
.getfname
	...
	ld	de,filename
.PZ1	LD 	A,(HL)		;GET CHARACTER FROM EXPRESSION
	AND	A		;TEST FOR END OF EXPRESSION
	JP	M,pzend		;IF SO, END IT NOW
	ld	(de),a
	INC	HL		;POINT TO NEXT
	inc de
	jr	PZ1		;KEEP GOING TILL END
.pzend
	xor	a
	ld	(de),a	;Fix string termination
	ret

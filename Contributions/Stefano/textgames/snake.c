...
int main()
{
  ...
  do /* restart game loop */
  {
    ...
    do
    {
      ...
      csleep(speed*pause_length); /*pause*/
      ...
      if (firstpress) { getk();getk(); while(!kbhit()); firstpress = 0; direction = right_key;}
      //if (firstpress) { while(getk()); while(!getk()); firstpress = 0; direction = right_key;}
      ...
      if (screen_grid[snake[snake_length-1].row-2+maxrow*(snake[snake_length-1].col-2)]=='o')
      {
        ...
        {
          score+=level*100; obstacles+=2; level++;  /* add to obstacles */

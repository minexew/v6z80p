;--------------------------------------------------------------------------------------------------
; Low level Z80 MMC/SDC/SDHC card access routines - Phil Ruston '08-'12
;--------------------------------------------------------------------------------------------------
;
; V1.13 - Reformatted source to Tab size 8
; V1.12 - (Mods by Stefano Bodrato) Different workaround for SD quirk and real CRC7 checksum generation for commands
...
; ------
;
; The main body of code is MAINLY hardware agnostic, however the READ SECTOR and WRITE SECTOR loops
; are optimized for the V6Z80P (generic versions are supplied below each, currently commented out).
; Also, the first byte from the card following a "send_command" is skipped for the V6Z80P interface.
; This is because "D_out" is not pulled up, meaning the first byte of data from the card following a
; command is undefined for one byte. This may or may not be an issue for other designs.
;
; Does not check for the block size offered by card: Assumes 512 byte sector access.
;
...
;-------------------------------------------------------------------------------------------------
; STANDARDIZED DRIVER HEADER FOR V6Z80P / FLOS 
;-------------------------------------------------------------------------------------------------
...
;-------------------------------------------------------------------------------------------------
; MAIN Z80 SD_CARD DRIVER CODE BEGINS
;-------------------------------------------------------------------------------------------------
...
sd_ecilp	call sd_send_eight_clocks	
		...
		call sd_send_command_null_args		; (When /CS is low on receipt of CMD0, card enters SPI mode) 
...
;..................................................................................................	
;		ld b,0					; unoptimized sector read (512 bytes) 
;		call sd_read_bytes_to_sector_buffer
;		inc h
;		ld b,0
;		call read_bytes
...
;..............................................................................................	
;		ld hl,(sector_buffer_loc)		; unoptimized sector write (512 bytes) 
;		ld b,0
;sd_wslp	ld a,(hl)
;		call sd_send_byte
;		inc hl
...
; This data can be placed in ROM if desired:

CMD8_string		db $48,$00,$00,$01,$aa
ACMD41HCS_string	db $69,$40,$00,$00,$00
...
;---------------------------------------------------------------------------------------------
; V6Z80P OSCA Specific Hardware Level Routines v1.10
;---------------------------------------------------------------------------------------------

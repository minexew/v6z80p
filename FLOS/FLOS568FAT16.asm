...
;
; A Note about VRAM paging on VxZ80P
; ----------------------------------
...
kjt_enable_pointer		jp os_enable_pointer	;start + $8b
...
;---------------------------------------------------------------------------------
	...
	ld a,$c3			; sets $C3 (JP) instruction
	ld (irq_jp_inst),a		; at IRQ vector locations in RAM
	ld (nmi_jp_inst),a
	ld hl,os_irq_handler	; set IRQ vectors 
          ld (irq_vector),hl	  
	ld hl,os_no_nmi_freeze	 
	ld (nmi_vector),hl	 

	xor a
	out (sys_audio_enable),a	; disable audio channels
			
	in a,(sys_vreg_read)	; enable NMI switch
	bit 5,a			; 60Hz mode?
	jr z,pal50hz			
	ld a,%00000100		; maintain 60Hz mode selection
...
paltv	ld (hl),e			; set y window size/position (200 lines)
	...
	ld hl,pointer_colours	; copy 8 colours for sprite pointer to end of palette 
	ld de,palette+(248*2)	
	ld bc,16
	ldir
	ret
...
os_bchl_memfill
	...
	push de			; fill memory from HL with A. Count in BC.
	ld e,a			
bchlloop	ld (hl),e
	inc hl
	dec bc
...
posmatch	ld a,(iy-1)		; if command string char is a space, the command matches
	...
	jr nc,ic_nohwer		; on return from INTERNAL COMMAND, is carry set (driver error)? 

os_hwerr	ld hl,hex_byte_txt		; If so, show drive error bits and return
...
ic_nohwer	or a			; if a = 0 after internal command, no error report: return normally
	ret z			
ic_nohwe2	cp $fe			; if command wants to spawn a new command, return now
	ret z
	ld b,a			; the program reported an error - show the error message
	ld c,0
...
os_gecmd	ld hl,(os_args_start_lo)	; Found external command!
	...
	ld ix,0
	ld iy,8
	call os_set_load_length	; load the first 8 bytes into a buffer
	ld hl,scratch_pad
	ld (fs_z80_address),hl
	call fs_read_data_command
	jp c,os_hwerr		; hardware error?
	or a
	jp nz,ic_nohwer		; file sys error?
	ld hl,(scratch_pad)
...
readcode	ld (os_extcmd_jmp_addr),hl	; store code execution address
	...
	jp c,os_hwerr		; drive error?
	or a
	jp nz,ic_nohwer		; file system error?
...
extcmderf	jr z,not_errc		; if ZERO FLAG is set, the program completed OK
	or a
	jr z,accuzero		; if A = 0 and zero flag is not set, there was hardware error
	cp $ff
	jp z,os_cold_start
	jp ic_nohwe2
accuzero	ld a,b			; get the code from B and report it
	jp os_hwerr

not_errc	cp $ff			; no error but check for a = $ff on return anyway (OS needs to restart..)
	jp z,os_cold_start		; we're in a subroutine, but SP is initialized on cold start so no worries.
	ret			
...
os_cfne	ld (os_args_start_lo),hl	;update arg position for next parameter
	ld a,c			
	or a			;a=number of chars in filename
	ret
...
os_enable_pointer
	...
	ld (mouse_window_size_x),hl	; hl/de = window size mouse pointer is to work within
	ld (mouse_window_size_y),de
	push bc			; bc = address of pointer palette (colours 248-255)
	pop hl
	ld a,%10000000		; put pointer colours below video registers
	out (sys_alt_write_page),a
	ld de,pointer_colours	; copy 8 colours for sprite pointer to buffer 	
	ld bc,8*2
	ldir
	xor a
	out (sys_alt_write_page),a
	ld a,1
	ld (use_mouse_pointer),a	; OS to use pointer
	xor a
	ld (mouse_packet_index),a
	ld a,%10000011
	out (sys_irq_enable),a	; enable mouse/keyboard interrupts
	ret


os_get_mouse_position

; returns xpos in HL, y pos in DE, buttons in A
...
;---------------------------------------------------------------------------------------
; Commonly called error messages
;---------------------------------------------------------------------------------------
...
os_no_fn_error	xor a
		ld a,$0d
		ret

os_fn_too_long	xor a
		ld a,$15
		ret
	
os_no_start_addr	xor a
		ld a,$16
		ret

os_no_filesize	xor a
		ld a,$17
		ret
...
os_invalid_bank	xor a
		ld a,$1b
		ret
	
os_no_e_addr_error	xor a
		ld a,$1c
		ret

os_no_d_addr_error	xor a
		ld a,$1d
		ret
	
os_range_error	xor a
		ld a,$1e
		ret

os_no_args_error	xor a
		ld a,$1f
		ret	

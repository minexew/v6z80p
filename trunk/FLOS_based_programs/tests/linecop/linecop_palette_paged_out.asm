...
; Palette/ROM is paged out - should not affect linecop
...
mcblp	ld (ix+0),$00			;set reg 0
	...
	di				; disable interrupts cus ROM is paged out too
	ld a,$40
	out (sys_alt_write_page),a		; page out ROM/palette

	ld e,0				; test CPU writes to sys mem "under" the palette
bloop	ld hl,$0				; whilst ROM is paged out.
	ld bc,$1ff
lp1	ld (hl),e
	ld a,(hl)
	inc hl

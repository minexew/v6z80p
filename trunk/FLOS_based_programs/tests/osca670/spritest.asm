...
testlp1	call kjt_wait_vrt
	...
	ld a,h
	or $10			; bit 7 = reset, this sprite's colours are in normal range
	ld (ix+1),a		; sprite should be masked by background colours 128-255
	pop hl
	inc hl
	ld a,h
	cp 2
...
testlp2	call kjt_wait_vrt
	...
	ld a,h
	or $90
	ld (ix+1),a		; bit 7 = set: force this sprite's colours high on this pass
	pop hl			; sprite should appear in front of all background colours
	inc hl			; sprite colour should change on this pass
	ld a,h
	cp 2
	jr nz,testlp2
	ret
...
testlp3	call kjt_wait_vrt
	...
	ld (ix+1),a		; bit 7 = set: force this sprite's colours high on this pass
	pop hl			; sprite should appear in front of all background colours
	inc hl			; sprite colour should be same as pass 1
	ld a,h
	cp 2
	jr nz,testlp3
	ret

...
;--------------------------------------------
;
; Custom Priority registers set, but priority toggle is switched off
; therefore bit 7 of the height/ctrl reg should make no difference
; to the priority between the two passes. Instead it should set the
; height of the sprite (normal operation - priority requires the
; source sprite pixels themselves to be set accordingly)
;
...
test1	ld a,%00000011		; Bit 6 = keep colours between 0-127? (off)
	ld (vreg_sprctrl),a		; Bit 4 = use priority toggle? (off) Bit 1 = priority interleave? (on)
...
testlp1	call kjt_wait_vrt
	...
	ld a,h
	or $10			; bit 7 = reset, sprite uses priority mask set 0
	ld (ix+1),a		
	pop hl
	inc hl
	ld a,h
...
test2	ld a,%00000011		; Bit 6 = keep colours between 0-127? (off)
	ld (vreg_sprctrl),a		; Bit 4 = use priority toggle? (off) Bit 1 = priority interleave? (on)
...
testlp2	call kjt_wait_vrt
	...
	ld (ix),l			; move sprite across screen (update x coord)
	ld a,h
	or $90			; bit 7 = set: sprite uses priority mask set 1
	ld (ix+1),a		
	pop hl			
	inc hl			
	ld a,h

...
; Somewhat arbitrary timimg due to quirks of my SD interface ("D_out" not pulled up)
...
; sd_read_sector	[sector_lba]		Zero Flag / A = error code
; sd_write_sector	[sector_lba]    		Zero Flag / A = error code
;
...
; Define "sd_sector_buffer" in main code (512 bytes)
...
sd_spi_mode_ok
	...
	ld b,4
	call sd_read_bytes_to_sd_sector_buffer	; get r7 response (4 bytes)
	ld a,1
	inc hl
	inc hl
...
;------ SDHC CARD IS INITIALIZED --------------------------------------------------------------------------------------
	...
	ld b,4				; read in OCR
	call sd_read_bytes_to_sd_sector_buffer
	ld a,(hl)
	and $40				; test CCS bit
	rrca
	rrca 
...
sd_id_ok	ld b,18				; read the card info to sector buffer (16 bytes + 2 CRC)
	...
	ld ix,sd_sector_buffer		; compute card's capacity
	bit 6,(ix)
	jr z,sd_csd_v1
...
; Returns HL = Pointer to device ID string
	...
	ld b,18
	call sd_read_bytes_to_sd_sector_buffer	; read 16 bytes + 2 CRC
	
	ld hl,sd_sector_buffer+$03		; Build name / version / serial number of card as ASCII string
	ld de,sd_sector_buffer+$20
	ld bc,5
	ld a,(sd_card_info)
	and $f
	jr nz,sd_cn5
...
sd_hvl2	ld (de),a
	...
	ld hl,sd_sector_buffer+$20		; Drive (hardware) name string at HL
	xor a
	ret
...
sd_dt_ok	ld hl,sd_sector_buffer		; optimized read sector code
	ld c,sys_spi_port
	ld b,0
	ld a,$ff
...
sd_write_sector_main
	...
	ld hl,sd_sector_buffer			; optimized write sector code
	ld c,sys_spi_port
	ld b,$00
...
;..............................................................................................	
;	ld hl,sd_sector_buffer		; write out 512 bytes for sector -unoptimized
;	ld b,0
;sd_wslp	ld a,(hl)
;	call sd_send_byte
;	inc hl
...
sd_set_sector_addr
	...
	ld bc,(sector_lba+2)
	ld hl,(sector_lba)			; sector LBA BC:HL -> B,D,E,C
	ld d,c
	ld e,h
	ld c,l
	ld a,(sd_card_info)
...
sd_read_bytes_to_sd_sector_buffer
...
; The following variables need to placed in RAM:

cmd_generic	db $00
cmd_generic_args	db $00,$00,$00,$00
cmd_generic_crc	db $01

sector_lba 	db 0,0,0,0	; Sector address (little endian)

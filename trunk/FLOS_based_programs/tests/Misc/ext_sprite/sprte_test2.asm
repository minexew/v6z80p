...
;--------------------------------------
; Test #2 force bit 7 (colours 128-255)
;--------------------------------------
...
wsprrlp 	ld (hl),0
	...
	ld a,%00010011
	ld (vreg_sprctrl),a		; Bit 4 = force bit 7, 1 = priority interleave, 0 = Enable sprites
...
nocarry	rlca			; use one of the high bits of the count to cycle force bit 7
	rlca
	rlca
	and $80			; bit 7 is the force colour 128+ select 

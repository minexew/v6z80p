;********************************************************
;* Sound FX Editor for FLOS V0.24 - By Phil Ruston 2010 *
;********************************************************
...
fx_display_width	equ 40
fx_display_height	equ 25

window_width_pixels	equ fx_display_width*8
marker_y		equ 136
...
old_flos_txt
	...
	db "Program requires FLOS v563+",11,11,0
...
script_editing
	...
	ld hl,(scr_edit_pos)		;backspace chars in buffer
	ld a,(scr_start_offset)
	add a,l
	ld l,a
	ld c,h
	xor a
	or c
	jp z,delete			;if pressed backspace at column 0
	ld h,0
	add hl,hl
	add hl,hl
	add hl,hl
...
scrbslp1	ld a,(hl)
	...
	call kjt_get_cursor_position		;backspace chars on screen
sebslp	call kjt_get_charmap_addr_xy
	ld a,(hl)
	dec b
	call kjt_plot_char
	inc b
	...
	call kjt_get_cursor_position		;shift cursor back	
	dec b
	ld a,c
	sub 5
	ld c,a
...
insline	ld hl,(scr_edit_pos)	;insert a blank line (move everything down a place)
	ld a,h
	or a
	jr z,pushdown		;if cursor is not at col 0, push down from below
	ld h,0
	ld (scr_edit_pos),hl
	inc l
pushdown	ld a,(scr_start_offset)
	ld d,0
	ld e,a
	add hl,de
	add hl,hl
...
notm2pe	ld a,(ascii_entry_mode)	;already entering data in a DEL box?
	...
	ld c,a
	ld b,0
	ldir			;save the original data from the box / button

	bit 6,(ix+2)
	jr nz,accept_da		;just a button activation?
...
flag2	call clear_wave_image	;clip selection changed, clear the wave image
	call no_markers
	jr flag7
...
pre_snd	ld a,(current_wave)		
	...
	ld a,(ix)			;is this sample defined?
	dec a
	ld l,a
	ld h,0
	add hl,hl
...
ffcocmd	ld a,(hl)			;get byte from edited text
	...
	inc hl
	djnz ffcocmd
	jp gonxtl			;if all chars are spaces, go to next line
...
tryrest	ld a,(ix)			
	or a
	jr z,thiscmd		;reached a zero in list of commands = its this command
	cp (hl)
	jr nz,notthcmd		
...
notthcmd	inc ix			;move to next command - find following zero
	...
	ld a,(ix+2)		
	cp $ff			;Last command? (First ascii byte of next command = $ff)
	jr nz,findcmd
...
show_comp_errors
	...
	push af				;draw blank script error box
	ld b,0
	ld c,10
	call kjt_set_cursor_position
	ld hl,script_err_box_txt
	call kjt_print_string
	ld b,7
blankllp	push bc
	...
	ld hl,huh_txt
	cp 1
	call z,unkcmde			;1 = unknown command
	cp 2
	call z,scr2bige			;2 = no done
	cp 3
	call z,hexbade			;3 = bad hex
	cp 4
	call z,malfe			;4 = malformed line
	call print_string
	ld hl,(comp_line)
	call print_string
	ret
...
pload_er2	call kjt_print_string	
	call wait_two_seconds
	ret

proj_exists

	ld de,proj_filename		; update project filename
	ld bc,12
	ldir
...
skipopscr	
	...
	ld c,2
	ld hl,opt_data_fn		
	call save_requester			;bring up requester
	call charmap_restore
	jr z,save_opt_data
	ld hl,aborted_txt
	cp $ff
	jr z,sopdabort
	ld hl,disk_error_txt
sopdabort	call kjt_print_string	
	call wait_two_seconds
	ret
...
optimized_samples_save
	...
	ld hl,opt_data_fn			; copy optimized data filename to optimized samples
	ld b,8				; filename up to the "." and add ".SAM" on the end
	ld de,opt_samples_fn
cpyfnlp	ld a,(hl)
	cp "."
	jr z,cafnfd
	ld (de),a
...
cafnfd	ex de,hl
	...
	ld hl,opt_samples_fn		; if this file exists, delete ti
	call kjt_erase_file
	
	ld ix,$8000			; create the samples file - save first 2 zero bytes
	ld b,3				; bank = 3
	ld de,2				; len = 2
	ld c,0
...
lsreq_ok	pop de			; copy filename from requester to current sample name location
	push hl
	ld bc,12
	ldir
	pop hl
...
sdisk_err	ld hl,disk_error_txt		
sload_err	ld b,0
	ld c,21
	call kjt_set_cursor_position
	call kjt_print_string	
...
wavabort	ld hl,sload_aborted_txt
	jr sload_err
...
sfxed_txt	db "SOUND FX EDITOR 0.24 - TOP LEVEL        ",0
...
waved_txt	db "SOUND FX EDITOR 0.24 - WAVE SELECTOR    ",0
...
sample_load_txt
	...
	db "SOUND FX EDITOR 0.24 - SAMPLE LOADER    ",0
...
sload_aborted_txt	db "Aborted Sample Load!",0
disk_error_txt	db "Disk Error!",0
...
project_loadsave_txt
	...
	db "SOUND FX EDITOR 0.24 - DATA LOAD/SAVE   ",0

loadsavepage_txt

	db 11,"Full FX Editor Project:"
	db 11,"-----------------------",11
	db 11," [LOAD] .fxp file",11
	db 11," [SAVE] .fxp file",11,11,11
	
	db 11,"Optimized Files for FX Player:"
	db 11,"------------------------------",11
	db 11," [SAVE] .bin & .sam files",0
	

proj_filename	db "PROJECT1.FXP",0
opt_data_fn	db "MY_FX1.DAT  ",0
opt_samples_fn	db "            ",0,0
...
script_ed_txt
	...
	db "SOUND FX EDITOR 0.24 - SCRIPT EDITOR    ",0
...
script_err_box_txt	db "-------------------------------------",11,0
blank_line_txt	db "                                     ",11,0
huh_txt		db "Uknown error!",11,"Weirdness :",0		
...
fix_abandon_txt	db 11,11,"Press:",11,11
		db "[A] to Abandon the edited script",11
		db "    or any other key to fix it.",11,0

sure_txt		db 11,11,"Sure? (Y/N)",11,11,0

argbyte_txt	db " [byte]",0
argword_txt	db " [word]",0
...
comp_error1_txt		db 11,11,"Error: Re-numbering clips",0
comp_error2_txt		db 11,11,"Error: Re-numbering samples",0
comp_error3_txt		db 11,11,"Error: Re-numbering script CLIP cmds",0
optdatasave_error_txt	db 11,11,"Error saving optimized data!",0
opt_data_saved_txt		db 11,11,"Optimized data saved OK..",0
opt_samples_saved_txt	db 11,"Relevant samples saved OK...",0
no_fx_txt			db 11,11,"No FX to save!",0
optimizing_txt		db "Optimizing",0
...
charmap_image		ds fx_display_width*fx_display_height,0

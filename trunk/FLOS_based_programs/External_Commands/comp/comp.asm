...
; App: Comp - compares files. V1.00 By Phil Ruston
...
fn2done	ld hl,filename1		;does file 1 exist?
	...
	sbc hl,de
	jr z,fslen
notslen	ld hl,not_same_len_txt
	call kjt_print_string
...
bufcmplp	ld a,(de)
	...
	push de
	push bc
	call kjt_get_key		;quit long lists with ESC key
	pop bc
	pop de
	pop hl
	cp $76
...
all_done	ld hl,done_txt
	...
	ld a,(diff_flag)		;set error $80 if different, $00 if same
	rrca
	or a
	ret
...
usage_txt		db 11,"COMP.EXE - Compare files",11,11
		db "Usage: COMP FILE1 FILE2",11,11,0

not_same_len_txt	db "Files are not the same length.",11
		db "Will compare to end of shortest.",11,0
		
comparing_txt	db "Comparing..",11,0

done_txt		db "Done",0
identical_txt	db " - Files are identical.",0

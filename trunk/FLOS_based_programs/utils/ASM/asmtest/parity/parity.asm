...
;--------- Load 4096 colour pic data --------------------------------------------
	...
	ld hl,$8000		; Z80 address to load to 
	call kjt_force_load
	jp nz,load_error
...
;--------- Load samples for tune -------------------------------------------------
	...
	call kjt_find_file
	jp nz,load_error
	ld b,3			; bank 3 (audio ram base, IE: $20000)
	ld hl,$8000		; Z80 address to load to 
	call kjt_force_load
	jp nz,load_error
	ld hl,0
...
load_err_txt	db "Load error. File not found?",11,0
...
bgnd_colours	incbin "back_pal.bin"

font_colours	incbin "font_pal.bin"

;------------------------------------------------------------------------------------------

	include	"Protrack.asm"

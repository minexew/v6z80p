; ****************************************************************************
; * ONBOARD EEPROM MANAGEMENT TOOL FOR VxZ80P V1.11 - P.Ruston '08 - '09     *
; ****************************************************************************
...
option_1	call show_banner
	...
	cp 1			; if uploading to slot 1 show warning
	jr nz,ok_to_wr
...
cfgloaded	ld a,0			; is is a Xilinx cfg file?
	...
	add a,$30
	ld (erase_char),a
	ld hl,erasing_text		; show "erasing" text
	call kjt_print_string
...
option_7	call show_banner
	...
	ld hl,block_prompt_text		; write data to block - ask what block..
	call kjt_print_string
	call kjt_get_input_string
	or a
	jp z,aborted
	...
	cp 1
	jr z,selpzero
	ld hl,cfg_warning_txt
	call kjt_print_string
	call kjt_get_input_string		; and ask for confirmation
	or a				; if selected block > 1
	jp z,aborted
	ld a,(hl)
	cp "Y"
	jp nz,aborted
...
start_text1	db 11," ************************************ ",11
		db    " * VxZ80P ONBOARD EEPROM TOOL V1.11 * ",11
		db    " ************************************ ",11,0
		
start_text2	db 11
		db    "          Select an option:",11
		db    "          -----------------",11,11
		...
		db 11
		db    "7 - Insert data into EEPROM page",11
		db 11
		db    "ESC - Quit",11,11,0
		
slot_prompt_text	db 11,11,"Write new config to which slot? ",0

warning_1_text	db 11,11,"WARNING! You have chosen to re-write",11
		db "slot 1 which by default holds the",11
		db "OSCA system architecture.",11,11
		db "Sure you want to proceed? (y/n) ",0
		
download_text	db 11,"Please start serial download (ESC=Quit)",11,0

receiving_text	db 11,"Receiving:",0
...
verifying_text	db "Verifying data",11,0
...
block_prompt_text	db 11,"Write to which 64KB block? (0-7) ",0
...
set_slot_text	db 11,11,"From which slot should the FPGA",11
...
warning_2_text	db 11,11,"WARNING! You are changing the power",11
		...
		db "from EEPROM slot $0"
check_digit	db "x - Sure? (y/n) ",0
		
reconfig_now_text	db 11,11,"Reconfigure FPGA now (non permanent)"
		db 11,11,"Which slot? ",0

ok_text		db 11,11,"Completed OK - Press any key",11,0
...
update_bootcode_txt	db 11,"Update Bootcode..",11,11
		db "Update primary [0] bootcode",11
		db "or backup [1] version? (0/1) ",0
...
cfg_warning_txt	db 11,11,"Warning! This will overwrite one of the",11
		db "FPGA configuration slots. Are you",11
		db "sure you want to proceed? (y/n) ",0

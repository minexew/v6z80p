...
; By Phil Ruston 2010 - Version : V(6)30 for V6Z80P+ (Incompatible with original V6Z80P)
; ----------------------------------------------------------------------------------------
;
; New in v(6)30
; -------------
; Manual slot selection now reads EEPROM ID to obtain number of slots, allowing any available slot to be set.
; Removed I2C code (as programmble clock chip was removed from PCB design on V6Z80P+ 1.1)
; Removed Test Commands
; Added EEPROM set memory protection command, read status byte command, read programming permission command
; Waits for EEPROM busy and EEPROM WEL now time out
; Some code optimizaion
;
; ----------------------------------------------------------------------------------------------------------------------
;
;  Pin connections:
...
; $88 + $8b + $xx = write $xx to EEPROM's Status Register (set protection bits)*
; $88 + $06 = Request EEPROM sends its Status Register (read the protection bits)
...
; ************************************************************************************************************************
; Manual slot reset system: In JTAG mode, remove jumper and replace it when the LED has indicated desired slot
; then power off, remove jumper and power on - desired slot will be active. 
; ************************************************************************************************************************
...
unused_a			equ 4   ;out (Open Drain - pin is not used)
...
unused_b			equ 5	;out (pin is not used)
jumper				equ 6	;in
...
porta_default		equ (1<<eeprom_cs)						; EEPROM not selected
portb_default		equ (1<<fpga_program)					; FPGA program high (inactive)
...
configure_fpga
			...
			call deselect_eeprom								; Ensure SPI EEPROM is not selected
			movlw portb_default									; Set FPGA program line inctive
			movwf PORTB											
			call short_pause
...
;---------- Manual active slot reset system --------------------------------------------------------------------------------
			...
			movlw spi_id_cmd									
			call send_byte_to_eeprom							; send "get eeprom ID" command ($ab) to SPI EEPROM 											
			movlw 0
			...
			movlw 4												; if EEPROM ID read fails for some reason, allow slots 1-3 to be selected
			movwf max_slot
			movf received_byte,w								; convert EEPROM ID to available slots
			sublw 0x10
			skipifnotborrow
			goto mset_slot
			sublw 0x08
			skipifborrow
			goto mset_slot
			addlw 0x09											; ID range limit: $10-$17, else use default 4 slot selection
			clrf max_slot
			incf max_slot,f
findmaxslot	sublw 0x01
			skipifnotzero
			goto mset_slot
			bcf STATUS,C										;clear carry flag
			rlf max_slot,f										;double slot count
			btfss max_slot,4									;if max_slot => 16, max_slot=16 (just cus any more flashes are hard to count)
			goto findmaxslot
...
do_clocks equ (1<<eeprom_clock)+(1<<fpga_cclk)
...
wait_cmd_lp	call get_byte_from_fpga								; all commands must begin with $88
			...
			movf received_byte,w						;added in (6)30
			xorlw 0x8b
			skipifnotzero
...
read_status_register
			...
			call send_byte_to_eeprom
			goto send_byte_direct_from_eeprom
...
return_programming_permission
			...
			andlw 0x01											; allow/disallow programming commands) - Note this command
			call send_w_to_fpga 								; sends data over PIC COMMS, not direct from EEPROM
			goto wait_command
...
send_firmware_vers
			...
			movlw firmware_vers									; Returns firmware version code
			call send_w_to_fpga									; Note: This command sends data over PIC COMMS, not direct from EEPROM
			goto wait_command
...
idbit_lp	movlw portb_default+(1<<comms_clock_out)		   ; set clock hi = latch bit
			...
			movwf PORTB
			movlw (1<<eeprom_clock)							   ; set SPI EEPROM clock high
			movwf PORTA					
...
db_timeout	call deselect_eeprom								; deselect the SPI EEPROM (set its /CS = high)
			goto wait_command
...
erase_eeprom_block
			...
			call send_byte_to_eeprom
			call deselect_eeprom								; deselect the SPI EEPROM (set its /CS = high: end of instruction)
			...
			call deselect_eeprom								; deselect the SPI EEPROM (set its /CS = high) -  initiates the EEPROM's internal programming operation
			call long_pause
...
writeb_loop	movf INDF,w
			...
			call select_eeprom									; verify the 64 bytes just written - Bring SPI EEPROM /CS = Low (new instruction)
			movlw spi_read_cmd									; send read data command ($03) to SPI EEPROM 
			call send_byte_to_eeprom						
...
ver_error	call deselect_eeprom								; If the 64 bytes read back do not match those in PIC's memory
			movlw 12											; flash the LED 12 times, keeping the PIC clock out line high
			goto flash_error


cant_program
			
			movlw 2												; LED flashes twice to warn not in programming mode
			goto flash_error									; PIC clock out line is held high for duration
...
flash_error	movwf busy_flash									; flash the LED 10 times, keeping the PIC clock out line high
...
wel_set		call deselect_eeprom								; deselect the SPI EEPROM (set its /CS = high: end of instruction)
			return
...
wip_loop	call get_byte_from_eeprom							; read status byte from EEPROM
			...
			decfsz temp3,f
			goto wip_loop
busy_error	call deselect_eeprom								; deselect the SPI EEPROM (set its /CS = high: end of instruction)
			bsf time_out,0
			return		

not_busy	call deselect_eeprom								; deselect the SPI EEPROM (set its /CS = high: end of instruction)
			return

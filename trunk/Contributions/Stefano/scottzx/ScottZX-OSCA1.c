...
Compile command:
	...
	zcc +osca -oscottzx.exe -D__Z88DK__ -lflosdos -lmalloc ScottZX-OSCA.c
 */
...
#else
#define set_pen(a) asm("nop\n");
#define flos_paper(x) asm("nop\n");
#endif
...
#include "Scottzx.h"
	...
	#define NO_Z88DK_HEAP
#endif

#ifdef __OSCA__
	// 20480(org)+16400(size) = 36900
	#define HPSIZE 25000	// up to abt 52000, there's still space for stack
	extern char heap[] @ 37000
	#define NO_Z88DK_HEAP
#endif

#ifndef NO_Z88DK_HEAP
	// max space available on Spectrum with direct Microdrive control driver
	#define HPSIZE 19000
	HEAPSIZE(HPSIZE)
#endif
...
*/
	...
	if(argc!=2)
	{
		printf("\nusage: SCOTTZX <database game file>\n");
		exit(1);
	}
	f=fopen(argv[1],"r");
	if (f == NULL) {
		printf("\nCannot open game database file.\n");
		exit(1);
	}
...
*/
	...
	//printf ("%c[2J",27);
	printf ("\
Scott Free, A Scott Adams game driver\n\
written in C language.\n\
Release 1.14, (c) 1993,1994,1995 \n\
Swansea University Computer Society.\n\
Distributed under GNU software license\n\n\
Z88DK OSCA port v1.1 by S. Bodrato\n\n");

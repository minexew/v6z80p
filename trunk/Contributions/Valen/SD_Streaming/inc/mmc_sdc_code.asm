...
; ------------
;
; Sector read / write code have optimized 512 byte transfer loops
;
; Limitations:
; ------------
...
mmc_get_id
	...
	call kjt_get_version		; Check that V5Z80P hardware version has SPI interface
	ld hl,$263-1			
	xor a
	sbc hl,de
	jr c,msdhw_ok
...
mmc_init_done
	...
	ld hl,sector_buffer			; read the card info to sector buffer
	ld b,16
mmc_csdlp	call mmc_get_byte
	...
	ld hl,sector_buffer+16		; read in more card data 
	ld b,16
mmc_cidlp	call mmc_get_byte
	ld (hl),a
	inc hl
	djnz mmc_cidlp
	call mmc_get_byte			; read CRC byte 1
...
;set c:de to sector number to write, 512 bytes written from sector buffer
	...
	ld a,$ff				; wait 8 clocks before proceding
	call mmc_send_byte			
	ld a,$fe
...
;	djnz mmc_wslp
	...
	ld a,$ff				; send dummy CRC byte 1 
	call mmc_send_byte
	ld a,$ff				; send dummy CRC byte 2
	call mmc_send_byte
...
mmc_spi_port_slow
	...
	out (sys_sdcard_ctrl1),a		; Forces expansion pins 0+1 = output, 4= input
	ret
...
mmc_wait_4ms
	...
	push af
	xor a				; set timer to count 256 x 16 microseconds 
	out (sys_timer),a			; before setting the IRQ flag
	ld a,%00000100			
	out (sys_clear_irq_flags),a		; clear IRQ flag before wait
mmc_twait	in a,(sys_irq_ps2_flags)			
	bit 2,a				; wait for timer IRQ flag to become set..
	jr z,mmc_twait			
	pop af
	ret

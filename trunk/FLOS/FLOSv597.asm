...
tst_fkey	cp (hl)
	...
	ld hl,key_txt		; not found, try yo change to "%key" envar dir assignment
	call cd_envar
...
fkey_gfn	xor a			; OK, found key file!
	ld hl,OS_window_cols	
	call set_loadlength24	; limit load size
	ld hl,commandstring		
	push hl
...
set_loadlength24
	...
	ld (fs_file_length_temp),hl		; set load length to A:HL
	ld h,0
	ld l,a
...
os_set_load_length
	...
	ld (fs_file_length_temp),iy		; set load length to IX:IY (for kernal)

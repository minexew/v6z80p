...
;----------------------------------------------------------------------------------------------
;
; Changes:1.20 - Fixed missing EOF error introduced in v118
...
fs_load	call test_bytes_to_go
	jr nz,fs_btrok
fs_fliz	xor a				
	ld a,$07				; error $07 - requested transfer length is zero
	ret
 
fs_btrok	ld hl,(fs_z80_address)		; Get load address 
	ld (fs_z80_working_address),hl	; (routine affects working copy only)

	ld bc,(fs_sector_pos_cnt)		; Set BC in case of a load continuation

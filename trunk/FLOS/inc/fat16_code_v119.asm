...
;----------------------------------------------------------------------------------------------
;
; Changes:1.19 - Changed error codes $25 to $2e (removed: $26 as it was never used)
...
at_pbs	xor a
	ld a,$2e				; return code $2e no partition info (MBR) on this device
	ret
...
fs_load	call test_file_length_temp
	jr nz,fs_btrok
...
fs_fpok	ld bc,(fs_sector_pos_cnt)		; Set BC in anticipaiton of a continued load
	ld a,(fs_filepointer_valid)		; if the file pointer has been changed, we need
	or a				; to seek again from start of file.
	jr nz,fs_prfld			; otherwise restore jump back into the main load routine
...
fs_getfilesec
	...
	call test_file_length_temp		; is file transfer length = 0?
	jr nz,fs_prfld

fs_flesc	ld (fs_sector_pos_cnt),bc		; if so back up regs for any following sequential read
	xor a				; op completed ok: a = 0, carry = 0
	ret

fs_prfld	push bc				; stash sector pos / countdown
	ld de,(fs_in_sector_offset)
	ld hl,512
	xor a
	sbc hl,de
	ld b,h
	ld c,l				; bc = number of bytes to (attempt to) read from sector
	
	ld hl,sector_buffer			
	add hl,de				; add in_sector_offset to sector base
	
	ld de,(fs_z80_working_address)	; get dest address for file bytes



;-------- MAIN SECTOR TO DEST ADDR LOOP: PRE-CALC SPEED OPTIMIZED in V118 -----------------------------------------------

; HL = source address in sector buffer
; DE = dest address in Z80 address space
; BC = transfer bytes (to end of sector buffer)

; Would the proposed block transfer (to the end of sector buffer) exceed the desired byte count?

	push hl				;protect HL source address 
	ld hl,(fs_file_length_temp)	
	xor a
	sbc hl,bc			
	ld (fs_file_length_temp),hl		;remaining file length LSW after this operation
	jr nc,okto_eos	
	ld hl,(fs_file_length_temp+2)
	dec hl
	ld (fs_file_length_temp+2),hl		;remaining file length MSW after this operation
	ld a,h
	and l
	inc a
	jr nz,okto_eos			;is file transfer countdown < 0?
	ld hl,(fs_file_length_temp)
	add hl,bc
	ld b,h				;Yes: so set BC to the original remaining file length 
	ld c,l			
	ld hl,0
	ld (fs_file_length_temp),hl		;Set transfer length **REGISTERS** to zero
	ld (fs_file_length_temp+2),hl
okto_eos	pop hl				;restore HL source address


	
	push hl				;add the real transfer length to the file pointer
...
; Do the transfer - if split by a bank increment, do in two LDIRs
	...
	jr z,fs_memok			;if last bank, return with bank error
	pop bc
	or a				
	ret
...
fs_btfnobi
	...
	ld (fs_z80_working_address),de	;update working destination address
	...
	ld a,h	
	or l				; if the LDIR ops didn't end at start of a new sector - stop here,
	jp nz,fs_flesc			; saving the sector-in-cluster position for subsequent reads


;-------- END OF MAIN SECTOR BUFFER TO DEST LOOP --------------------------------------------------------------------------


	inc c				; next sector
	dec b
	jp nz,fs_getfilesec			; loop until all sectors in cluster read
	...
	jp c,fs_nfbok			
	call test_file_length_temp		; that's OK if we're at the end of the file transfer req
	ret z 
	jp fs_fpbad			; if not, return EOF error
...
test_file_length_temp
	
	ld hl,(fs_file_length_temp)		; is file transfer count now zero?
	ld a,h
	or l
	ld hl,(fs_file_length_temp+2)
	or h

...
kjt_background_colours	jp os_null		;start + $b8 (retired in v593)
...
packed_font	incbin "philfont3_128chars_packed.bin"
char_colour	db 0
...
clrvlp1	ld (hl),$ff		; Fill $18000-$19fff = $ff (paper bitplane)	
	...
	jr z,clrvlp1
	xor a
clrvlp2	ld (hl),a			; Fill rest of VRAM 64KB page $1A000=$1fff = $00
	inc hl
	bit 7,h
	jr nz,clrvlp2
	out (sys_mem_select),a	; vram paged out / default upper RAM bank is 0

	call os_page_in_video	; prepare font in VRAM (@ $1E000-$1F000) for char plot routine		
	ld hl,packed_font		; unpack font to VRAM (@ $1E400)
	ld de,video_base+$400
	push de
	ld bc,end_of_font-packed_font
	call unpack_rle
	pop bc			; make inverse charset (@ $1E800)
	ld de,video_base+$800	; and fill $1ec00-$1efff with $ff
...
mult_cursor_y_window_cols
	...
	ld a,(cursor_y)			; returns cursor y * os_window_cols in HL
	ld h,a
	xor a
	ld l,a
	ld (mult_index),a
...
os_main_loop
	...
	ld de,$43f		; normal underscore cursor
	ld a,(insert_mode)
	or a
	jr z,undsc_c
	ld de,$c00		; full block cursor
...
tst_fkey	cp (hl)
	...
	call os_check_volume_format	; System looks on the ACTIVE SELECTED drive only
	jr nz,nofkstr		; make sure disk is available
	ld hl,fkey_filename
	call os_find_file		; is there are function key command string in this folder?
	jr z,fkey_gfn
	call os_root_dir		; no, so go to root/keymaps and look there
	ld hl,keymaps_txt
	call os_change_dir
	jr nz,nofkstr
	ld hl,fkey_filename
	call os_find_file		; look for file called Fx.CMD (where x if relevent f-key)
	jr nz,nofkstr		; if no filename for this f-key, do nothing
fkey_gfn	ld ix,0
	ld iy,OS_window_cols	
	call os_set_load_length	; limit load size
	ld hl,commandstring		
	push hl
	ld bc,OS_window_cols	; clear command string
	ld a," "
	call os_bchl_memfill
	pop hl
	push hl
	ld b,0
	call os_force_load		; load text string to scratch pad
	pop hl
	jr nz,nofkstr
	call os_print_string
	call restore_dir_block	; go back to original dir
	jp gotcmdstr		; treat string as an entered command
nofkstr	call os_restore_dir
	jp os_main_loop
...
os_parse_cmd_chk_ps
	...
	jr z,newxcmd
	ld hl,master_commandstring	; has a master command been set up?
	ld a,(hl)
	or a
	ret z
newxcmd	call copy_to_command_string	; max string length = width of window in chars
...
not_g_cmd	ld (os_args_start_lo),hl	; attempt to load external OS command
	...
	call os_check_volume_format	; System looks on the ACTIVE SELECTED drive only
	jr nz,os_ndfxc		; make sure disk is available
	
	ld hl,scratch_pad
	call os_find_file		; test file exists in current dir
	jr z,os_gecmd		; 0 = got external command

	call os_root_dir		; cant find it, so go to root dir
	ld hl,commands_txt		; and look in "COMMANDS" dir if it exists
	call os_change_dir		
	jr nz,os_ndfxc		; Show "unknown command" if that dir isnt there
	ld hl,scratch_pad
	call os_find_file		; try to find the command in the new dir
	jr z,os_gecmd
...
cntuasr	call setup_mult_table	; (AF is preserved by this routine)
	...
	push af			; Set "ERR" envar to exiting program's error code
	ld de,0
	ld (scratch_pad+2),de
...
ui_loop	ld de,$43f		;force underscore cursor
	call cursor_keywait

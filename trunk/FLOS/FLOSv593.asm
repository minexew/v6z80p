...
;-------------------------------------------------------------------------------
	...
	ld a,7			
	call set_bitplane
	ld a,%00100000		; wipe VRAM $10000-$1ffff
...
paltv	ld (hl),e			; set y window size/position (200 lines)
	...
	ld ix,priority_registers	; Set the sprite priority registers to the default scheme
	ld b,8			;
sprprilp1	ld (ix),%00		;
	ld (ix+8),%01		;
	inc ix			;
...
hexbyte_to_ascii
	...
	push bc			;puts ASCII version of hex byte value in A at HL (two chars)
	ld b,$f
	ld c,a			;then hl = hl + 2
	rrca
	rrca

;----------------------------------------------------------------------------------------
; Convert Amiga hardware values from Z80 Protracker Player to EZ80P spec
; and writes to hardware registers
;-----------------------------------------------------------------------------------------
;
; V1.01 - Z80 mode (non-ADL code) for HW v0036
; Writes 4 channels to left and right stereo (no separation)
;----------------------------------------------------------------------------------------
...
fourthchan_location	equ channel_data+(vars_per_channel*3)+samp_loc_lo
...
fourthchan_loop_loc	equ channel_data+(vars_per_channel*3)+samp_loop_loc_lo
fourthchan_loop_len	equ channel_data+(vars_per_channel*3)+samp_loop_len_lo

;----------------------------------------------------------------------------------------
; EZ80P specific code starts
;----------------------------------------------------------------------------------------

sound_registers		equ 0f80000h

hw_ch0_loc 			equ sound_registers+00h
hw_ch0_loop_loc 	equ sound_registers+20h
hw_ch0_len			equ sound_registers+40h 
hw_ch0_loop_len		equ sound_registers+60h
hw_ch0_per			equ sound_registers+80h
hw_ch0_vol			equ sound_registers+c0h

hw_ch1_loc 			equ sound_registers+04h
hw_ch1_loop_loc 	equ sound_registers+24h
hw_ch1_len			equ sound_registers+44h 
hw_ch1_loop_len		equ sound_registers+64h
hw_ch1_per			equ sound_registers+84h
hw_ch1_vol			equ sound_registers+c4h

hw_ch2_loc 			equ sound_registers+08h
hw_ch2_loop_loc 	equ sound_registers+28h
hw_ch2_len			equ sound_registers+48h 
hw_ch2_loop_len		equ sound_registers+68h
hw_ch2_per			equ sound_registers+88h
hw_ch2_vol			equ sound_registers+c8h

hw_ch3_loc 			equ sound_registers+0ch
hw_ch3_loop_loc 	equ sound_registers+2ch
hw_ch3_len			equ sound_registers+4ch 
hw_ch3_loop_len		equ sound_registers+6ch
hw_ch3_per			equ sound_registers+8ch
hw_ch3_vol			equ sound_registers+cch

hw_ch4_loc 			equ sound_registers+10h
hw_ch4_loop_loc 	equ sound_registers+30h
hw_ch4_len			equ sound_registers+50h 
hw_ch4_loop_len		equ sound_registers+70h
hw_ch4_per			equ sound_registers+90h
hw_ch4_vol			equ sound_registers+d0h

hw_ch5_loc 			equ sound_registers+14h
hw_ch5_loop_loc 	equ sound_registers+34h
hw_ch5_len			equ sound_registers+54h 
hw_ch5_loop_len		equ sound_registers+74h
hw_ch5_per			equ sound_registers+94h
hw_ch5_vol			equ sound_registers+d4h

hw_ch6_loc 			equ sound_registers+18h
hw_ch6_loop_loc 	equ sound_registers+38h
hw_ch6_len			equ sound_registers+58h 
hw_ch6_loop_len		equ sound_registers+78h
hw_ch6_per			equ sound_registers+98h
hw_ch6_vol			equ sound_registers+d8h

hw_ch7_loc 			equ sound_registers+1ch
hw_ch7_loop_loc 	equ sound_registers+3ch
hw_ch7_len			equ sound_registers+5ch 
hw_ch7_loop_len		equ sound_registers+7ch
hw_ch7_per			equ sound_registers+9ch
hw_ch7_vol			equ sound_registers+dch

;------------------------------------------------------------------------------------------------------------------
; Wait for "audio hardware registers updated" flag 
;------------------------------------------------------------------------------------------------------------------

update_audio_hardware

				ld a,1
				ld.lil (hw_ch0_loc+3),a				; enable playback / clear audio register status flag
				ld c,port_hw_flags
wait_audreg		tstio 40h							; wait for audio hardware to finish reading registers
				jr z,wait_audreg
				
				ld a,0ffh							; for TEST only
				ld.lil (hw_palette),a				; for TEST only
...
;---------------------------------------------------------------------------------------------------------------
				...
				ld hl,(firstchan_period)	  	
				add hl,hl
				add hl,de
				ld e,(hl)
				inc hl
				ld d,(hl)
				ld.lil (hw_ch0_per),de				;24 bit write mandatory
				ld.lil (hw_ch4_per),de
				
				ld de,period_conv_table-200		
				ld hl,(secondchan_period)	  	
				add hl,hl
				add hl,de
				ld e,(hl)
				inc hl
				ld d,(hl)
				ld.lil (hw_ch1_per),de				;24 bit write mandatory
				ld.lil (hw_ch5_per),de
		
				ld de,period_conv_table-200		
				ld hl,(thirdchan_period)	  	
				add hl,hl
				add hl,de
				ld e,(hl)
				inc hl
				ld d,(hl)
				ld.lil (hw_ch2_per),de				;24 bit write mandatory
				ld.lil (hw_ch6_per),de

				ld de,period_conv_table-200		
				ld hl,(fourthchan_period)	  	
				add hl,hl
				add hl,de
				ld e,(hl)
				inc hl
				ld d,(hl)
				ld.lil (hw_ch3_per),de				;24 bit write mandatory
				ld.lil (hw_ch7_per),de
...
;---------------------------------------------------------------------------------------------------------------
				...
				ld e,a
				ld.lil (hw_ch0_vol),de				;24 bit write mandatory
				ld.lil (hw_ch4_vol),de
				
				ld a,(secondchan_volume)
				ld e,a
				ld.lil (hw_ch1_vol),de				;24 bit write mandatory
				ld.lil (hw_ch5_vol),de
				
				ld a,(thirdchan_volume)
				ld e,a
				ld.lil (hw_ch2_vol),de				;24 bit write mandatory
				ld.lil (hw_ch6_vol),de
				
				ld a,(fourthchan_volume)
				ld e,a
				ld.lil (hw_ch3_vol),de				;24 bit write mandatory
				ld.lil (hw_ch7_vol),de
...
;---------------------------------------------------------------------------------------------------------------
				...
				ld e,l
				ld d,h
				ld.lil (hw_ch0_loc),de				;24 bit write mandatory
				ld.lil (hw_ch4_loc),de	
				ld hl,(firstchan_loop_loc)
				add hl,hl
				ld e,l
				ld d,h
				ld.lil (hw_ch0_loop_loc),de			;24 bit write mandatory
				ld.lil (hw_ch4_loop_loc),de	
				ld hl,(firstchan_length)
				add hl,hl
				ld e,l
				ld d,h
				ld.lil (hw_ch0_len),de				;24 bit write mandatory
				ld.lil (hw_ch4_len),de
				ld hl,(firstchan_loop_len)
				add hl,hl
				ld e,l
				ld d,h
				ld.lil (hw_ch0_loop_len),de			;24 bit write mandatory
				ld.lil (hw_ch4_loop_len),de
no_rt_ch0
				...
				ld e,l
				ld d,h
				ld.lil (hw_ch1_loc),de				;24 bit write mandatory
				ld.lil (hw_ch5_loc),de
				ld hl,(secondchan_loop_loc)
				add hl,hl
				ld e,l
				ld d,h
				ld.lil (hw_ch1_loop_loc),de			;24 bit write mandatory
				ld.lil (hw_ch5_loop_loc),de	
				ld hl,(secondchan_length)
				add hl,hl
				ld e,l
				ld d,h
				ld.lil (hw_ch1_len),de				;24 bit write mandatory
				ld.lil (hw_ch5_len),de
				ld hl,(secondchan_loop_len)			
				add hl,hl
				ld e,l
				ld d,h
				ld.lil (hw_ch1_loop_len),de			;24 bit write mandatory
				ld.lil (hw_ch5_loop_len),de	
no_rt_ch1
				...
				ld e,l
				ld d,h
				ld.lil (hw_ch2_loc),de				;24 bit write mandatory
				ld.lil (hw_ch6_loc),de
				ld hl,(thirdchan_loop_loc)
				add hl,hl
				ld e,l
				ld d,h
				ld.lil (hw_ch2_loop_loc),de			;24 bit write mandatory	
				ld.lil (hw_ch6_loop_loc),de	
				ld hl,(thirdchan_length)
				add hl,hl
				ld e,l
				ld d,h
				ld.lil (hw_ch2_len),de				;24 bit write mandatory	
				ld.lil (hw_ch6_len),de	
				ld hl,(thirdchan_loop_len)
				add hl,hl
				ld e,l
				ld d,h
				ld.lil (hw_ch2_loop_len),de			;24 bit write mandatory
				ld.lil (hw_ch6_loop_len),de	
no_rt_ch2

				ld hl,fourthchan_control
				bit 0,(hl)
				jr z,no_rt_ch3
				res 0,(hl)
				...
				ld e,l
				ld d,h
				ld.lil (hw_ch3_loc),de				;24 bit write mandatory
				ld.lil (hw_ch7_loc),de	
				ld hl,(fourthchan_loop_loc)
				add hl,hl
				ld e,l
				ld d,h
				ld.lil (hw_ch3_loop_loc),de			;24 bit write mandatory
				ld.lil (hw_ch7_loop_loc),de	
				ld hl,(fourthchan_length)
				add hl,hl
				ld e,l
				ld d,h
				ld.lil (hw_ch3_len),de				;24 bit write mandatory
				ld.lil (hw_ch7_len),de
				ld hl,(fourthchan_loop_len)
				add hl,hl
				ld e,l
				ld d,h
				ld.lil (hw_ch3_loop_len),de			;24 bit write mandatory
				ld.lil (hw_ch7_loop_len),de
no_rt_ch3

				ld a,0h								; for TEST only
				ld.lil (hw_palette),a				; for TEST only

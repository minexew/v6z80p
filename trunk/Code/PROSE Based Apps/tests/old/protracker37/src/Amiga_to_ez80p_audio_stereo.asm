...
;-----------------------------------------------------------------------------------------
;
; V1.02 - Z80 mode (non-ADL code) for HW v0037
; Writes tracker channel 0 to HW01
;    					 1 to HW23
;                        2 to HW45
;                        3 to HW67
;----------------------------------------------------------------------------------------
...
update_audio_hardware
				...
				ld a,1
				ld.lil (hw_audio_registers+3),a		; enable playback / clear audio register status flag
				ld c,port_hw_flags
wait_audreg		tstio 40h							; wait for audio hardware to finish reading registers
				jr z,wait_audreg
				
				ld a,0ffh							; for testing only
				ld.lil (hw_palette),a				; for testing only
				
;---------------------------------------------------------------------------------------------------------------
; Write Period Constants - must be done in max 1500 50mhz clocks
;---------------------------------------------------------------------------------------------------------------
								
				ld.lil ix,hw_audio_registers		; start of channel 0 audio registers
				ld.lil iy,hw_audio_registers+80h	; start of channel 4 audio registers
				...
				inc hl
				ld d,(hl)
				ld.lil (ix+08h),de					;update chan 0 - 24 bit write mandatory
				ld.lil (ix+28h),de					;copy to chan 1 also
				...
				inc hl
				ld d,(hl)
				ld.lil (ix+48h),de					;update chan 2 
				ld.lil (ix+68h),de					;copy to chan 3 also
				...
				inc hl
				ld d,(hl)
				ld.lil (iy+08h),de					;update chan 4 
				ld.lil (iy+28h),de					;copy to chan 5 also
				...
				inc hl
				ld d,(hl)
				ld.lil (iy+48h),de					;update chan 6 
				ld.lil (iy+68h),de					;copy to chan 7 also
...
;---------------------------------------------------------------------------------------------------------------
				...
				ld e,a
				ld.lil (ix+0ch),de					;update chan 0 - vol - 24 bit write mandatory
				ld e,a
				ld.lil (ix+2ch),de					;copy to chan 1 also
...
;---------------------------------------------------------------------------------------------------------------
				...
				add.l hl,hl
				ld.lil (ix),hl						;update chan 0 - loc (24 bit write mandatory)
				ld.lil (ix+20h),hl					;copy to chan 1 also
				ld.lil hl,0
				ld hl,(firstchan_loop_loc)
				add.l hl,hl
				ld.lil (ix+10h),hl					;update chan 2 - loop loc
				ld.lil (ix+30h),hl					;copy to chan 1 also
				ld.lil hl,0
				ld hl,(firstchan_length)
				add.l hl,hl
				ld.lil (ix+04h),hl					;update chan 4 - len
				ld.lil (ix+24h),hl					;copy to chan 1 also
				ld.lil hl,0
				ld hl,(firstchan_loop_len)
				add.l hl,hl
				ld.lil (ix+14h),hl					;update chan 6 - loop len
				ld.lil (ix+34h),hl					;copy to chan 1 also
no_rt_ch0

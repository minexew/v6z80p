;------------------------------------------------------------------------------------------------------
; Spectrum 128 / 48 Emulator Bootcode.exe
; Note: This uses its own filesystem routines, not those in FPGA boot ROM
;------------------------------------------------------------------------------------------------------
;
; Changes: v014 - stops reading dirs at last entry (faster dir list)
;               - SDHC compatible SD card driver added
;
;-------------------------------------------------------------------------------------------------------
...
rom_restart_128_snapshot	equ $68			; in FPGA ROM
rom_restart_48_snapshot		equ $6b			; in FPGA ROM

stack	    			equ $3fe0

sna_address   			equ $3fe5		; IE: $4000 less 27 bytes (.sna header)

rom_tag				equ $ffe0		; $D1CEFACE here if system previously booted
...
;-----------------------------------------------------------------------------------------------
		...
		ld de,$D1CE				; (look for $D1CEFACE ID)
		xor a
		sbc hl,de
		jr nz,no_prior
		ld hl,(rom_tag+2)
...
no_prior	xor a
		...
		call reset_keyboard			; Warning! "Reset keyboard" zeros port 254
		jr nc,kbrsok
		ld hl,kb_err_txt			; Warn if keyboard error
		call print_string

kbrsok		call sd_initialize
		jp c,card_init_error
		
		call fat16_check_format
		jp nz,card_form_error
...
no128rom	ld hl,rom48_fn				; load Spectrum 48 16KB ROM to $4000-$7FFF in buffer mem 02
		ld de,$4000
		call fat16_open_file
		jr nz,no48rom
		xor a
...
dumsnah		ld (hl),0				; clear .sna header
		...
		ld (restart_jp+4),a
		jp rom_restart_128_snapshot		; restart .sna - note: A Jump not a Call
...
notadir		ld (filename_loc),hl
		ld de,restart_pc			; attempt to load the first four bytes of additional Spectrum 128 .sna info
		...
		call fat16_read_data			; if load succeeds, go to 128KB .sna loading section
		jp z,load128

		ld a,(roms_available)			; This is an orignal Spectrum 48KB snapshot.
		bit 1,a					; Was the Spectrum 48 ROM loaded?
		...
		ld bc,$7ffd
		out (c),a
		jp rom_restart_48_snapshot		; restore registers etc with ROM in FPGA
...
get_hw_version
		...
		ld c,252				; port to read from				
		ld b,15					; get HW vers as hex word
verloop3	in a,(c)				; serial data is bit 7
		sla a					; force into carry flag
		adc hl,hl				; copy bit 0 of hl	
		djnz verloop3				; next bit
		ld (hw_vers),hl
		ld a,h
		or l
		ret z					; if hw vers = 0, dont overwrite version text
		
		ld hl,hw_vers_txt			; get hardware version (4 digit BCD)
		ld b,15					; bit number to read
		ld d,4					; number of digits
verloop2	ld e,4					; bits per digit
...
card_init_error	ld hl,sd_init_error_txt			; Note: print_string requires spectrum RAM selected
		jp show_error
		
card_form_error	ld hl,sd_format_error_txt
		jp show_error
	
show_card_error	ld hl,hex_txt
...
sd_init_error_txt
		...
		db "SD CARD INIT FAILED",11,11,0

sd_format_error_txt

		db "UNRECOGNIZED SD CARD FORMAT",11,11,0

fat_error_txt	db "FILE SYSTEM ERROR: "
hex_txt		db "xx",11,11,0
 
;----------------------------------------------------------------------------------------------------------

include "Alternative_Configs\Phil\Spectrum_Emulator\z80_code\inc\reset_keyboard.asm"

include "Alternative_Configs\Phil\Spectrum_Emulator\z80_code\inc\spectrum_fat16_load_essentials.asm"

include "Alternative_Configs\Phil\Spectrum_Emulator\z80_code\inc\spectrum_sdcard_essentials.asm"

include "Alternative_Configs\Phil\Spectrum_Emulator\z80_code\inc\debug.asm"
...
greetings_txt 		db " SPECTRUM 48,128 EMULATOR V0014",11,11,0
...
kb_err_txt		db "KEYBOARD ERROR",11,11,0

hw_vers			dw 0

hw_vers_txt		equ greetings_txt+27

;----------------------------------------------------------------------------------------------
; Simplified Z80 FAT16 File System routines ** LOAD ONLY **
;----------------------------------------------------------------------------------------------
;
; Notes: Z80 Address wraps around to $c000 for Spectrum 128
;        Call "sdc_initialize" and "fat16_check_format" before any other routine
;
;----------------------------------------------------------------------------------------------
;
; Return codes:
...
diskid_ok
	ld a,(fs_sector_buffer+$3a)		; must be FAT16, char at $36 should be "6"
	cp $36
	jr nz,test_mbr
...
fmaxcl	srl a
	...
	jr fmaxcl
got_cmaxc
	push hl				;if max clusters > $ffef, truncate to $fff0
	ld de,$fff0
	xor a
	sbc hl,de
	jr c,cmaxok
...
fs_parent_dir_command
	...
	ret
pdnaroot
	ld hl,fs_sought_filename		;make filename = "..         "
...
fs_btrok
	ld hl,(fs_z80_address)			; set load address 
...
fs_fpgbo
	add hl,bc				;HL = cluster's *byte* offset
	ld c,h
...
fs_dadok
	...
	jp nc,fs_fpbad			
fs_nfbok
	ld c,0					;following clusters have zero sector offset		
	ld a,(fs_cluster_size)	
...
fs_fniaf
	ld l,(ix+$1e)				;on return IX:IY = filesize
	ld h,(ix+$1f)
	ld e,(ix+$1c)
	ld d,(ix+$1d)
	push hl
...
fs_fpino
	ld hl,(fs_file_length_temp)		;countdown = 0?
	ld a,h
	or l

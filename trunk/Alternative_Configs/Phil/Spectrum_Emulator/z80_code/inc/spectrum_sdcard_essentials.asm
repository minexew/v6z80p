...
sd_id_ok  ld b,18                                 ; read the card info to sector buffer (16 bytes + 2 CRC)
          ...
          ld ix,fs_sector_buffer                  ; compute card's capacity
          bit 6,(ix)
          jr z,sd_csd_v1
...
; Returns HL = Pointer to device ID string
          ...
          ld hl,fs_sector_buffer+$03              ; Build name / version / serial number of card as ASCII string
          ld de,fs_sector_buffer+$20
          ld bc,5
          ld a,(sd_card_info)
          and $f
          jr nz,sd_cn5
...
sd_hvl2   ld (de),a
          ...
          ld hl,fs_sector_buffer+$20               ; Drive (hardware) name string at HL
          xor a
          ret
...
pause_4ms
	  ...
	  push af
	  push bc
	  ld bc,2461				; 2461 loops of (26 x 16MHz) = ~4mS		
	  ld a,(hw_vers+1)
	  and $f0
	  cp $30
	  jr nz,twait4ms
	  ld bc,538				; if PCB version = $3xxx assume CPU is 3.5MHz (v6+v1.1 14MHz Clock)
twait4ms  dec bc		
	  ld a,b
	  or c
 	  jr nz,twait4ms

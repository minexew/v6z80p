;----------------------------------
; OSCA Bootcode by Phil Ruston 2010
;----------------------------------

; Changes in V6.12
; ----------------
; Routines that were originally in ROM (some called via RST instructions) are now in bootcode
;  instead (EG: system initialization, "print string" etc). Bootcode is now totally independant of ROM
; Function keys F1-F7 select FPGA configs (was only F1-F3 before)
; Removed PQFS support routines 
; 
; --------------------------------------------------------------------------------------------------
;
; This boot code initializes the keyboard and loads an operating system file (*.OSF file)
; from MMC/SD card, EEPROM (location $00800+) or via serial download using the PC Serial Link app.
...
welcome_txt	db "V6Z80P BY PHIL RUSTON 2008-2010",11,11,"BOOTCODE:V6.12 - OSCA HW:V",0
...
start_boot
	...
	call init_vregs			; a call keeps the code out of unpaged memory
...
; This font data is OK here as it is immediately copied to VRAM before any sector buffer shenannigans
;*****************************************************************************************************

...
; By Phil Ruston '08-09. 8N1, no hardware flow control.
; V2.5 - Updated to use Purebasic's native serial library
; V2.7 - Unlimited load buffer size
...
EndEnumeration
  ...
  If OpenWindow(#Window_0, 314, 107, 319, 210,  "VxZ80P Serial File Transfer V2.7", #PB_Window_SystemMenu | #PB_Window_TitleBar)
...
EndIf
 ...
 EndIf
If findcom > 0
 SetGadgetState(#Combo_0,findcom-2)
EndIf
...
EndIf
 ...
 Repeat
 Until AvailableSerialPortInput(0) => 2 Or Date() > start_time+3 
 If Date() > start_time+3 
   moretext$="Timed out."
   Gosub more_text
   error=1
   Return
 ...
 ReadSerialPortData(0,*blockbuffer,2)                         ; ack should be "OK" or "XX"  
...
; FirstLine = 403
; Folding = -
; Executable = ..\V5Z80P_Serial_link_v2_6.exe
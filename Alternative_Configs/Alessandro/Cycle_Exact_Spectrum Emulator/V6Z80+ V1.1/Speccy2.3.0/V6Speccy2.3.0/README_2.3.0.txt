V6Z80P Speccy core 2.3.0 README
===============================

This is the binary distribution of V6Z80P Speccy core, a ZX Spectrum 48/128K/Pentagon 128K FPGA core for the V6Z80P+ v1.1
...
- Pentagon timing (48.83Hz vertical refresh, standard Pentagon refresh!)
- CPU Clock running at 3.5 standard mode / 7MHz turbo mode
- Fully uncontended mode for Pentagon emulation, both in 3.5 and 7.0MHz modes
- TV and VGA output
- ULA+ 64 color mode
...
The core can emulate only one DivMMC external interface.
You can enable the interface closing the "ExpB" jumper (please, take a look at the
following figure as reference: http://wiki.retroleum.co.uk/liberty/view/file/81).

When ExpB is OPEN, a virtual tape-player will be emulated and one TAP file can be loaded using the standard LOAD"" basic command.
...
at the prompt. Press "1" and identify 3 free slots. Enter the number of the first free slot.
When the file selector appears, select the 48K ".bin" config file you have previously copied on the SD card.
Repeat the EEPROM flash with the 128K ".bin" config file, placing it into the second slot. Finally,
repeat the EEPROM flash with the PNTG ".bin" config file, placing it into the third slot.
 (Once installed you can delete the .bin files from the root of the SD card if you wish).
...
Those files can be selected using the EMU.EXE program at the core startup or using exsDOS features.


Joystick
--------

You also can attach a joystick to the V6Z80P joystick port "A"; this joystick will be seen as a Kempston joystick.
...
feature, you need the ESXDOS.NVR file (contained in the "support files" distribution directory) in the
/SPECTRUM directory of the SD Card. Furthermore, you also need the directories "BIN" and "SYS" (and their
contents - also in "support files") in the / (ROOT) directory of your SD Card. (All these files are already
in place in the standard V6Z80P project folder: V6_SD_CARD.)
...
(Please use EMU version 0.08 or above. Requires FLOS v608+)

From version 2.0.0, the core should be started using a FLOS program developed by Phil Ruston called
EMU.EXE. This enables the user to start 48K, 128K and Pentagon 128K versions of the Speccy core
in a simple way. The program is included in the UTILS directory of the standard V6_SD_CARD .svn project
folder but is supplied in this archive too (under "Support files") - just copy it to UTILS, if
necessary, and it will be available from anywhere in FLOS.

From version 2.0.0 the emulators are started by running the EMU.EXE from command line. When entered without
arguments, the following menu appears:


* Boot the machine to BASIC (available only if ExpB jumper is OPEN)
 ----------------------------------------------------------------------------
  Selecting this option, the emulator will be started in standard ZX Spectrum 48/128K/Pentagon mode; no tapes,
  no ESXDOS installed. The version of the Speccy depends on the machine type selected using the Option 4.

* Load .tap /.bin file & boot (available only if ExpB jumper is OPEN)
  ------------------------------------------------------------------------------
...
* Boot into ESXDOS
  ---------------------------
  Selecting this option ESXDOS will be run at core startup.

   ExpB jumper OPEN => ResiDOS
   ExpB jumper CLOSED => Standard O.S. or Tape-player emulator.

C: Choose machine to emulate
   -------------------------
   Switch between the emulated machines (Spectrum 48, Spectrum 128, Pentagon 128)

S: Set emulator config slots
   -------------------------
...
M: Make the machine selection default
   ----------------------------------
   When you selec this option, the machine type currently choosen (by the "C" key) will be set as default at EMU startup.


The emulators can be booted more directly from the command line by entering the EMU command with parameters
as follows:

 "EMU M0" - boot Machine0, IE: 48 Spectrum
 "EMU M1" - boot Machine1, IE: 128 Spectrum
 "EMU M2" - boot Machine2, IE: 128 Pentagon

When run in this way the emulator will boot to an OS (Standard ROM or esxDOS, depending on the
EXP_B jumper status) If an emulator has not been run before, you'll be prompted to locate
its FPGA config file.
...
At the core startup, the CPU clock frequence will be set to 3.5xxMHz (standard ZX Spectrum 48/128K/Pentagon);
...
In Pentagon mode, on the other side, it will be recognized as "Turbo" mode :-)
...
PLEASE NOTE: the ULA+ is not enabled in Pentagon mode, to minimize the compatibility issues.
...
- SD Card support using the same logic (but different ports) in the ZXMMC+ interface by Alessandro Poppi

For a fast overview of the ESXDOS features, the original esxDOS 0.8.0 and 0.8.5 README is included in the
...
Some games will not run when using alternative Operating Systems such as esxDOS due to differences in the ROMs.
One workaround is to use the .tap preloader (option 2 in EMU.EXE), optionally employing the turbo mode (F2) to speed up loading.

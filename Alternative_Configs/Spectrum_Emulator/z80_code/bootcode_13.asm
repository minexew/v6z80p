...
;------------------------------------------------------------------------------------------------------
;
; Changes: v013 - Made universal for all V6Z80Ps
;
;-------------------------------------------------------------------------------------------------------
...
no_prior	xor a
	...
	jr nc,kbrsok
	ld hl,kb_err_txt		; Warn if keybiard error
	call print_string

kbrsok	call sdc_init
...
notadir	ld (filename_loc),hl
	...
	ld bc,$7ffd
	out (c),a
	jp kjt_restart_48_snapshot	; restore registers etc with ROM in FPGA
...
greetings_txt 	db " SPECTRUM 48,128 EMULATOR V0010",11,11,0

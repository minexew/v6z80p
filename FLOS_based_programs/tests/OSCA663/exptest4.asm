...
; Tests odd number of blocky pixels along a line with no modulo (data fetch address
; rounded up to an even address)
...
;-------------------------------------------------------------------------------
	...
	ld a,2
	ld (vreg_yhws_bplcount),a	; expanded pixel size = 3
	...
	xor a	
	ld hl,0			; each line has "85.333" blocky pixels
lp2	ld b,86			; this is rounded up to 86 bytes per line
lp1	ld (hl),a
	inc hl
	djnz lp1

...
; Tests Extended Tile Mode - Single Playfield, 16x16 tiles - Dual Playfield test
...
;----------------------------------------------------------------------------------------------
	...
	ld a,%10000001		
	ld (vreg_vidctrl),a		; select 16x16 tile mode / dual pf / normal border 
...
;---------------------------------------------------------------------------------------------------
;copy 32 tile block definitins to VRAM $00000
;---------------------------------------------------------------------------------------------------
...
;---------------------------------------------------------------------------------------------------
;Write some map data to Tilemap 0, Playfield B
;---------------------------------------------------------------------------------------------------
	...
	ld hl,video_base+$400	; write in some map data
	ld e,$10			; first char = $010
	ld c,4
blp2	ld b,4
...
do_stuff	ld hl,timer
	...
	and 2
	or %00000001
	ld (vreg_ext_vidctrl),a	; flip priority bit
	ret
...
my_tiles2		incbin "16x16_tiles2.bin"

;------------------------------------------------------------------------------------------
; Serial Tests - Updated for OSCA 675 (14400 and 28800 baud enabled)
;------------------------------------------------------------------------------------------
...
serial_tests
		...
		inc a
		ld hl,baud_list_txt
		ld bc,end_baud_list-baud_list_txt
		cpir
		call kjt_print_string
...
sent_txt		db 11,11,"OK, String sent to serial port..",11,0

prompt_txt		db "Enter a string to send and press ENTER",11,11,0

serial_headertag	db "Z80P.FHEADER"		;12 chars

start_text		db "Serial Test Menu",11,11,0

menu_text		db 11,11,"Press:",11,11
			db "1. Change BAUD",11
			db "2. Test file transfer - receive",11
			db "3. Display individual bytes received",11
			db "4. Send text string to serial port",11,11
			db "ESC - Main test menu",11,11,0

waiting_text		db "Waiting for serial file transfer..",11,0

baud_is_txt		db "BAUD = ",0

baud_list_txt		db 1,"57600",0
			db 2,"115200",0
			db 3,"14400",0
			db 4,"28800",0
end_baud_list
...
checksum_text		db "Serial Error!",11,11,0

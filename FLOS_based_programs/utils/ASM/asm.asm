...
; Z80 Assembler for FLOS by Phil Ruston, v0.02
;
; changes: 0.02 - path allowed for source file in command line source filename
;                 paths on command line do not have to be %assigns
;                 paths allowed in INCBIN, INCLUDE statements
;                 INCDIR directive added (specify dir paths in source)
;
;
; Use: ASM file.asm [path] [path]
;
;======================================================================================
...
asm_go    ld a,(hl)                     	; examine argument text for source filename
          or a                          
          jr nz,got_fn
...
got_fn    ld (args_loc),hl
	  call extract_path_and_filename
          ld hl,path_txt
          call kjt_parse_path        	   	;change dir according to the path part of the string
          ret nz
	  ld hl,filename_txt
	  ld de,working_src_filename  	 	; copy to working filename string
          ld bc,13
	  ldir
	  call get_dir_vol			; and save working dir and volume
	  ld (working_src_vol),a
	  ld (working_src_cluster),de
	  ld (src_base_vol),a			; and project base dir/cluster
	  ld (src_base_cluster),de
	  
addcmdp	  ld hl,(args_loc)
...
no_cmdl_paths
	  ...
	  ld a,(src_base_vol)			; go to project's base dir/cluster for source filename
...
missing_filename_txt
          ...
          db 11,"Usage: ASM source.asm [path] [path]",11,0

app_txt   db 11,"ASM v0.02 for FLOS by Phil Ruston",11,0
...
add_incl_dir	push ix
		push hl
		push de
		push bc
		ld b,a
		ld hl,dirvol_include_list
ffeil		push hl
		pop ix
		ld a,(ix)
		cp b
		...
		ld a,(ix+2)
		cp d
		jr z,inclquit				;already in list, silently ignore
...
include_list_full_txt

	  db "Include list full",0
...
working_src_cluster	      dw 0

working_src_vol               db 0
...
stack_entry_size    equ 12+1+2+4+3                          ; filename,zero,line_count,filepointer,cluster,volume
...
dirvol_include_list ds 30,$ff				    ;vol(byte),cluster(word) 10 entries allowed
		    db $fe
;------------------------------------------------------------------------------------------
